...
class PulseGenerator:
    ...
    def add_gaussian_freq(self, width_f, central_f = 0, area_time = 1, polarisation = [1,0],field_or_intesity = 'field',sig_or_fwhm = 'sig',phase_taylor=[],shift_time = 0, unit = 'Hz'):
        ...
        polar_x,polar_y = self._normalise_polarisation(polarisation)
        pulse = area_time*np.exp(-(self.frequencies-central_f)**2/(2*width_f**2))*np.exp(1j*self._Taylor(self.frequencies*2*np.pi,central_f*2*np.pi,coefficients=phase_taylor))#*1/self.dt 
    ...
    def add_rectangle_frequ(self,central_f = None, width_f = None, hight = 1,phase_taylor=[], polarisation = [1,0],shift_time = 0, unit = 'Hz'):
        # square pulse in Fourier space 
    ...
    def add_polynom_frequ(self,central_f, poly_f = [], phase_taylor=[],shift_time = 0, polarisation = [1,0], unit = 'Hz'):
        ...
        polar_x,polar_y = self._normalise_polarisation(polarisation)
        pulse *= np.exp(1j*self._Taylor(self.frequencies*2*np.pi,central_f*2*np.pi,coefficients=phase_taylor))
        pulse *= np.exp(1j*2*np.pi*self.frequencies*shift_time)#*self.dt # <---
        pulse_x = pulse*polar_x
        pulse_y = pulse*polar_y
        self._add_spectral(pulse_x,pulse_y)
        pass 
    ...
    def convolute_psf(self, width_f, gauss_power = 1, sig_or_fwhm = 'sig', field_or_intesity = 'field', polarisation = 'b', unit = 'Hz'):
        ...
        self.temporal_representation_x = np.fft.ifft(np.fft.ifftshift(self.frequency_representation_x)) / self.dt
        self.temporal_representation_y = np.fft.ifft(np.fft.ifftshift(self.frequency_representation_y)) / self.dt
    ...
    def _add_time(self, pulse_x_time, pulse_y_time):
        ...
        self.frequency_representation_x += np.fft.fftshift(np.fft.fft(pulse_x_time)) * self.dt
        self.frequency_representation_y += np.fft.fftshift(np.fft.fft(pulse_y_time)) * self.dt
    ...
    def _add_spectral(self, pulse_x_freq, pulse_y_freq):
        ...
        self.temporal_representation_x += np.fft.ifft(np.fft.ifftshift(pulse_x_freq)) / self.dt
        self.temporal_representation_y += np.fft.ifft(np.fft.ifftshift(pulse_y_freq)) / self.dt

        self._update_pulse_power()
        self._add_action_counter()
        pass

    def add_time_field(self,time,field, frequency = 0, polarisation = [1,0], unit = 'Hz', power = None): 
        pulse_interp = interpolate.interp1d(time,field,kind='cubic',fill_value=(field[0],field[-1]),bounds_error=False)
        pulse_out = self.copy_pulse()
        phase = self._Units(frequency, unit)
        
        pulse = pulse_interp(self.time)*np.exp(-1j*2*np.pi*phase*self.time)
        
        polar_x,polar_y = self._normalise_polarisation(polarisation)
        pulse_x = pulse*polar_x
        pulse_y = pulse*polar_y
        
        pulse_out._add_time(pulse_x,pulse_y)
        if power is not None:
            pulse_out.set_pulse_power(power)
        
        self.merge_pulses(pulse_out)
    ...
    def set_rotating_frame(self,new_rf = None, unit = 'nm'):
        ...
        self.frequency_representation_x = np.fft.fftshift(np.fft.fft(self.temporal_representation_x)) * self.dt
        self.frequency_representation_y = np.fft.fftshift(np.fft.fft(self.temporal_representation_y)) * self.dt
